...
ex.
filename=.\directory\datafile.csvLTC_normalized_vf_one_dose_us_cases2_scaled
movavg=7
xtitle=covid count
ytitle=covid deaths
...
ratio - get ratio of data2 over data1; 0 or 1

lagDepth - maximum lag time in days

dayIncrement - number of days between each row of data (1 for daily; 7 for
weekly)

lineWidth - width of date overlay line
...
for (i=1:length(paramFileNames))
    ...
    config.dayIncrement = getParamValue(params, "dayIncrement");
    config.lineWidth = getParamValue(params, "lineWidth");
    ...
    config.firstDate = datetime(config.firstDate, 'InputFormat', 'yyyy-MM-dd''T''HH:mm:ss.sss');
    ...
    % may need factor of 7 here for weekly data
    % note: probably not, since this uses daysact()
    if (config.startDayIndex ~= 1 && config.endDayIndex ~= length(config.axisData1))
        ...
        if (size(config.axisData2) > 0)
            config.axisData2 = config.axisData2(config.startDayIndex:config.endDayIndex);
        end
        if (size(config.axisData3) > 0)
            config.axisData3 = config.axisData3(config.startDayIndex:config.endDayIndex);
        end
    ...
    if (~isempty(config.movAvg))
        ...
        if (config.movAvg)
            ...
            config.axisData1 = movmean(config.axisData1, [config.movAvg 0]);
            config.axisData2 = movmean(config.axisData2, [config.movAvg 0]);
            config.axisData3 = movmean(config.axisData3, [config.movAvg 0]);
        end
    end
    ...
    if (isempty(config.dayIncrement))
        fprintf("day increment not found. default to 1 day\n");
        config.dayIncrement = 1;
    else
        config.dayIncrement = str2double(config.dayIncrement);
    end
    ...
    if (exist(config.datefile, 'file'))
        ...
        %config.dateDays/7 for weekly data (fixes overlays)
        config.dateDays = daysact( config.firstDate, dates )/config.dayIncrement;
        config.dateStrings = transpose(datetable.(2));
        if (size(datetable) > 2)
            config.dateColors = transpose(datetable.(3));
        end
    else
    ...
    if (~isempty(config.ratio))
        ...
        if (config.ratio)
            config.title2 = "(" + config.title2 + " / " + config.title1 + ")";
            config.title1 = "";
    ...
    % lineWidth default
    if (isempty(config.lineWidth))
        fprintf("no lineWidth found. defaultng to 1\n");
        config.lineWidth = 1;
    else
        config.lineWidth = str2double(config.lineWidth);
    end
...
function generatePlotThreeLines(config)
    ...
    if (isfield(config,"dateStrings"))
        for i=1:length(config.dateDays)
            if (isfield(config, "dateColors"))
                xline(config.dateDays(i), '-', {i}, 'HandleVisibility','off', 'LineWidth', config.lineWidth, 'Color', cell2mat(config.dateColors(i)));
            else
                xline(config.dateDays(i), '-', {i}, 'HandleVisibility','off', 'LineWidth', config.lineWidth);
            end
        end
    end
    ...
    for (i=1:size(ax.YAxis,1))
        ax.YAxis(i).Exponent = 0;
    end
    
    % xticks display 1st of each month
    % may need to deal with weekly here
    xdays = config.firstDate + [0:(config.endDayIndex - config.startDayIndex)*config.dayIncrement];
    fdom = xdays.Day == 1;
    marks = find(fdom);
    xticks(marks/config.dayIncrement);
...
function generateLagPlots(config)
    ...
    if (~isempty(config.axisData2))
        ...
        if (config.ratio)
            axisLimit1 = max(config.axisData2 ./ config.axisData1);
            axisLimit1 = 1.5 * axisLimit1;
            if (config.getLog)
                axisLimit1 = max(config.axisData2 - config.axisData1);
                axisLimit1 = 1.5 * axisLimit1;
            end
        ...
        for n = 0:config.lagDepth % lag in days
            ...
            elseif (config.plottype == "line")  % line plot
                if (config.ratio) % get ratio plot with single line
                    ...
                    if (config.getLog)
                    	ylim( [ -(abs(axisLimit1)), abs(axisLimit1) ] );
                    else
                        ylim([0 axisLimit1]);            
                    end
                    %ylim([0 5]);

                    if (~isempty(config.datefile))
                        for i=1:length(config.dateDays)
                            if (isfield(config, "dateColors"))
                                xline(config.dateDays(i), '-', {i}, 'HandleVisibility','off', 'LineWidth', config.lineWidth, 'Color', cell2mat(config.dateColors(i)));
                            else

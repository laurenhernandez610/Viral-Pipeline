...
%change these ("nationwide" in either variable to get nationwide data)
% leave county empty (counter = "";) for statewide data
county = "nationwide";
state = "";
...
while (~isempty(csvPage))
    offset = i*limit;
    if (county ~= "nationwide" && state ~= "nationwide")
        soqlStr = "?$$app_token=" + token + "&$select=" + select + "&$where=" + where + "&$group=" + group + "&$order=" + order + "&$offset=" + offset + "&$limit=" + limit;
    else
        soqlStr = "?$$app_token=" + token + "&$select=" + select + "&$group=" + group + "&$order=" + order + "&$offset=" + offset + "&$limit=" + limit;
    end
...
if (county ~= "nationwide" && state ~= "nationwide")
    ...
    outputFileName = "nursing_nationwide_" + string(date) + ".csv";
end
